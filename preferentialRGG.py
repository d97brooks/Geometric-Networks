...
def main():
    ## get graph requirements (size, radius)
    size = 0
    radius = 0
    size = int(sys.argv[1])
    ...
    print("Starting with Graph size " + str(starting))
    ## generate a starting graph
    G = nx.random_geometric_graph(starting, radius, 2)
    print("Starting with " + str(nx.number_of_edges(G)) + " edges")
    ...
    for n in range(size - starting):
        ...
        # for component in range(len(components)):
        #     #print( str( nx.number_of_nodes(components[component]) )  + " in component " + str(component + 1))
        #     probabilities.append( nx.number_of_nodes(components[component]) / s )
    ...
    print("Done.")
    print("Prefferential Attachment RGG Generation Time:" + "{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
    ...
    nx.write_gml(G, "prefRGG_" + str(size) + ".gml")
